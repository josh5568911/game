...
using namespace std;
int i, j, ss[200][150],Bse[200][150], Z = 0; int gg = 0; int voiceSize = 300; int soundSize = 300; int voSize = 300; int oms;/*F=時間計數器*/
long long int FP = 0;
clock_t ta=0, tb,start_time;
string s[26][60];
IMAGE mapP,lightP,text, back1, aline,player_image,enemyimage[10],save_image, BGBuffer;
HWND bc_l;
HANDLE hMutex;
...
int roll(string d, int a,mt19937 &rng) {
    /*1:nDn;2:1D20*/
    ...
    while (t--) {
        uniform_int_distribution<int> dist1(1, s);
        sum +=dist1(rng);
    }
    return sum + g;
}
...
public:
    wstring name, story;
    int lv, mhp, hp, dex, str, x, y, INT, con, wis, cha, speed, move, baid, exp,Move,pose,Act;
    int EDV = 0, DEF = 0;
};
class b_npc {
public:
...
class terrain {
public:
    int type, mA, mB,npc=0,player=0,enemy=0,fire=0,trap=0,dark=0,danger=0,pit=0, flare=0;
    /*dark:1=黑暗,2=夜視範圍,3=光照範圍*/
	int fire_time = 0; int flare_time = 0;
};
class t_equip {
public:
    wstring name,story;
...
void b_nput(b_npc* b_n, b_map* b_m, int nsize, int b_mid);
void e_put(enemy* e, b_map* b_m,terrain(*te)[50], int esize, int b_mid);
void maps(player* p, enemy* e, b_map* b_m,arms *ar, terrain(*te)[50], int b_mid);
void Redraw_battle(player* p, enemy* e, b_npc* b_n, arms* ar, terrain(*te)[50], b_map* b_m, int b_mid);
void b_camera( b_map* b_m,int x,int y, int b_mid);
void SEplay(string id, int type) {
    MIX_Audio* audio = nullptr;
    string ext;
    int ext_type = type % 10;
    int loops = type >= 10 ? -1 : 0;
    switch (ext_type) {
    case 1: ext = ".mp3"; break;
    case 2: ext = ".wav"; break;
    }
    string key = id + ext;
    auto IT = g_seCache.find(key);
    if (IT != g_seCache.end()) {
        audio = IT->second;
    }
    else {
        string path = "./Game/Sound/SE/" + key;
        audio = MIX_LoadAudio(g_mixer, path.c_str(), true);
        g_seCache[key] = audio;
    }
    if (loops == 0) {
        for (int I = 0; I < SE_TRACK_COUNT; I++) {
            if (MIX_TrackPlaying(g_seTrack[I]) &&
                MIX_GetTrackAudio(g_seTrack[I]) == audio) {
                return;
            }
        }
    }
    ...
    if (loops != 0) {
        g_loopingMap[id] = track;
    }
}
void SEstopLoop(string id) {
    auto IT = g_loopingMap.find(id);
    if (IT != g_loopingMap.end()) {
        MIX_StopTrack(IT->second, 0);
        g_loopingMap.erase(IT);
    }
}
void VOplay(string id,int type) {
    string ext;
    switch (type) {
    case 1: ext = ".mp3"; break;
    case 2: ext = ".wav"; break;
    ...
    string key = id + ext;
    MIX_Audio* audio = nullptr;
    auto IT = g_voCache.find(key);
    if (IT != g_voCache.end()) {
        audio = IT->second;
    }
    else {
        string path = "./Game/Sound/VO/" + key;
        audio = MIX_LoadAudio(g_mixer, path.c_str(), true);
        g_voCache[key] = audio;
    }
    for (int I = 0; I < VO_TRACK_COUNT; I++) {
        if (MIX_TrackPlaying(g_voTrack[I]) &&
            MIX_GetTrackAudio(g_voTrack[I]) == audio) {
            return;
        }
    }
    MIX_Track* track = nullptr;
...
void pHP(player* p,b_map *b_m,int id,int b_mid,int type,int &DMG) {
    ...
    if (type == 0) {
        ...
        /*減傷計算*/
		string  Dmg = to_string(DMG);
        wstring path= L"./Game/picture/hs.png";
		IMAGE da;
		loadimage(&da, path.c_str());
        BeginBatchDraw();
        int totalWidth = Dmg.size() * 20;
        for (int I = 0; I < Dmg.size();I++ ) {
            transparentimageA(NULL, p[id].x * 48 - b_m[b_mid].ox+I*20+24- totalWidth/2, p[id].y * 48 - b_m[b_mid].oy-40, &da,int(Dmg[I]-'0') * 20, 0, 20, 28);
        }
		EndBatchDraw();
        int dmg = DMG;
...
void eHP(enemy *e,b_map *b_m,int id,int b_mid,int type,int DMG) {
    if (type == 0) {
        ...
        int totalWidth = Dmg.size() * 20;
        for (int I = 0; I < Dmg.size(); I++) {
            transparentimageA(NULL, e[id].x * 48 - b_m[b_mid].ox + I * 20 + 24 - totalWidth / 2, e[id].y * 48 - b_m[b_mid].oy - 40, &da, int(Dmg[I] - '0') * 20, 0, 20, 28);
        }
        EndBatchDraw();
...
void nHP(b_npc *b_n,b_map *b_m,int id,int b_mid,int type,int DMG) {
    if (type == 0) {
        ...
        int totalWidth = Dmg.size() * 20;
        for (int I = 0; I < Dmg.size(); I++) {
            transparentimageA(NULL, b_n[id].x * 48 - b_m[b_mid].ox + I * 20 + 24 - totalWidth / 2, b_n[id].y * 48 - b_m[b_mid].oy - 40, &da, int(Dmg[I] - '0') * 20, 0, 20, 28);
        }
        EndBatchDraw();
...
void pD(wofstream* wofs,player*p,enemy *e,b_map *b_m,terrain(*te)[50],int P_id,int b_mid) {
...
void eD(wofstream* ,enemy* e,b_map *b_m,terrain(*te)[50],int id,int b_mid) {
...
bool checkBounds(b_map *b_m,int b_mid,int x,int y) {
    if (x<b_m[b_mid].x&&x>=0&&y<b_m[b_mid].y&&y>=0) {
        return true;
    }
    else {
        return false;
...
int retrye(wofstream* wofs, player *p,enemy *e,arms *ar,armor *Ar,b_map *b_m,int P_id,int id,int b_mid,int hcv,int type) {
    /*type0:普攻*/
    int ATK,AC;
    int ak[100];
    int K = 0;
        AC = roll("", 2,b_m[b_mid].rng) + (p[P_id].dex - 10) / 2 + Ar[p[P_id].armor_id].EDV;
        if (p[P_id].buff_check[27] == 1) {
            AC /= 2;
        }
        if (p[P_id].buff_check[14] == 1 || p[P_id].buff_check[20] == 1) {
    ...
    while (1) {
        ...
        if (ar[e[id].baid].type[0] == 'r') {
            ATK = roll("", 2, b_m[b_mid].rng) + roll(ar[e[id].baid].hit, 1, b_m[b_mid].rng) + (e[id].dex - 10) / 2;
        }
        else if (ar[e[id].baid].type[0] == 'm') {
            ATK = roll("", 2, b_m[b_mid].rng) + roll(ar[e[id].baid].hit, 1, b_m[b_mid].rng) + (e[id].str - 10) / 2;
        }
        }
        else if (type == 1) {
            ATK = roll("", 2, b_m[b_mid].rng)  + (e[id].str - 10) / 2;
        }
        if (hcv == 0) {
            if (ATK > AC) {
                *wofs << L",命中(" ;
...
int retryn(wofstream* wofs, b_npc* b_n, enemy* e, arms* ar, armor* Ar,b_map* b_m, int b_nid, int id,int b_mid, int hcv, int type) {
    /*type0:普攻*/
    int ATK, AC;
    int ak[100], ac[100];
    int K = 0, C = 0;        
        AC = roll("", 2, b_m[b_mid].rng) + (b_n[b_nid].dex - 10) / 2 ;
        if (b_n[b_nid].buff_check[27] == 1) {
            AC /= 2;
        }
        if (b_n[b_nid].buff_check[14] == 1 || b_n[b_nid].buff_check[20] == 1) {
            AC = 0;
        }
    while (1) {
        if (type == 0) {
...
int retryp(wofstream* wofs, player* p, enemy* e, arms* ar, armor* Ar, b_map* b_m,int P_id, int id,int ar_id,int b_mid, int hcv, int type) {
    int ATK, AC;
    int ak[100];
    int K = 0;
    string mark;
    AC = roll("", 2, b_m[b_mid].rng) + (e[id].dex - 10) / 2;
    if (e[id].buff_check[27] == 1) {
        AC /= 2;
    }
    ...
    while (1) {
        if (type == 0) {
        if (ar[ar_id].type[0] == 'r') {
            ATK = roll("", 2, b_m[b_mid].rng) + (p[P_id].dex - 10) / 2 + roll(ar[ar_id].hit, 1, b_m[b_mid].rng);
        }
        else if (ar[ar_id].type[0] == 'm') {
            ATK = roll("", 2, b_m[b_mid].rng) + (p[P_id].str - 10) / 2 + roll(ar[ar_id].hit, 1, b_m[b_mid].rng);;
        }
        }
        else if (type == 2) {
            ATK = roll("", 2, b_m[b_mid].rng) + (p[P_id].str - 10) / 2 + (p[P_id].dex - 10) / 2;
        }
        if (e[id].buff_check[19] > 0) {
            mark = "1d" + to_string(p[P_id].cha / 2);
            ATK += roll(mark, 1, b_m[b_mid].rng);
        }
        if (hcv == 0) {
            if (ATK > AC) {
                *wofs << L",命中(";
    ...
    if (chose[0] == 't') {
        ...
        for (int q = 0; q < p[I].buff_Size; q++) {
                        ...
                        p[I].hp = p[I].mhp;
                         ...
                         p[I].buff_check[6] += 2;                                        
                            ...
                            if (p[I].hp + p[I].mhp/4 > p[I].mhp) {
                                ...
                                mm =  to_string(p[I].mhp/4);
                                p[I].hp += p[I].mhp / 4;
                            }
                            p[I].act++;
                        }
                        wstring path = L"./Game/picture/ls.png";
                        IMAGE da;
                        loadimage(&da, path.c_str());
                        BeginBatchDraw();
                        int totalWidth = mm.size() * 20;
                        for (int J = 0; J < mm.size(); J++) {
                            transparentimageA(NULL, p[I].x * 48 - b_m[b_mid].ox+ 24 - totalWidth / 2, p[I].y * 48 - b_m[b_mid].oy - 40, &da, int(mm[J] - '0') * 20, 0, 20, 28);
                        }
                        EndBatchDraw();
                        Sleep(500);
          ...
          if (p[P_id].buff_check[31] > 0) {
              if (roll("", 2, b_m[b_mid].rng) > p[P_id].con) {
                  BeginBatchDraw();
          ...
          if (p[P_id].buff_check[36] > 0) {
			  p[P_id].buff_check[36]--;
              if (roll("", 2, b_m[b_mid].rng) > p[P_id].con) {
                  p[P_id].act--;
              }
          }
...
void n_buff_check(wofstream* wofs, string chose, player* p, enemy* e, b_npc* b_n, buff* bu, b_map* b_m, terrain(*te)[50], int P_id, int b_mid, int bu_id,int b_nid) {
    ...
    if (chose[0] == 'T') {
                        ...
                        if (b_n[I].buff_give[q][0] == 'i') {
                            ...
                            if (bt == 1) {
                                if (b_n[I].hp + 5 > b_n[I].mhp) {
                                    mm =to_string(b_n[I].mhp - b_n[I].hp);
                                    b_n[I].hp = b_n[I].mhp;
                                }
                                else {
                                    mm = to_string(5);
                            ...
                            int totalWidth = mm.size() * 20;
                            for (int J = 0; J < mm.size(); J++) {
                                transparentimageA(NULL, b_n[I].x * 48 - b_m[b_mid].ox + 24 - totalWidth / 2, b_n[I].y * 48 - b_m[b_mid].oy - 40, &da, int(mm[J] - '0') * 20, 0, 20, 28);
                            }
                            EndBatchDraw();
                            Sleep(500);
                            getimage(&get, 0, 0, 1296, 960);
        ...
        if (b_n[b_nid].buff_check[36] > 0) {
            b_n[b_nid].buff_check[36]--;
            if (roll("", 2, b_m[b_mid].rng) > b_n[b_nid].con) {
                b_n[b_nid].act--;
            }
        }
    }
        ...
        for (I = 0; I < b_m[b_mid].esize; I++) {
            for (int q = 0; q < e[I].buff_Size; q++) {
                if (e[I].buff_time[q] > 0) {
                    e[I].buff_time[q]--;
                    if (e[I].buff_id[q] == 35 && e[I].buff_time[q] % 3 == 0) {
                        int X, Y;
                            uniform_int_distribution<int> dist(-3, 3);
                        while (1) {
                            X = dist(b_m[b_mid].rng);
                            Y = dist(b_m[b_mid].rng);
                            if (abs(X )+abs(Y) < 4&&e[I].target_x + X >= 0 && e[I].target_x + X < b_m[b_mid].x &&e[I].target_y + Y >= 0 && e[I].target_y + Y < b_m[b_mid].y) {
                                break;
                            }
                        }
						SEplay("bomb",1);
                        if (te[e[I].target_x+X][e[I].target_y + Y].type != 1) {
                        ...
                        for (int ei = 0; ei < 6; ei++) {
                                putimage(0, 0, &get);
                                transparentimageA(NULL, 48 * e[I].target_x +48*X- b_m[b_mid].ox -36, e[I].target_y * 48+48*Y - b_m[b_mid].oy - 36, &ef, ei * 120,0, 120, 120);
                        ...
                        for (int rx = -1;rx <= 1;rx++) {
                            for (int ry = -1;ry <= 1;ry++) {
                                int tx = e[I].target_x + X + rx;
                                int ty = e[I].target_y + Y + ry;
                                if (tx < 0 || tx >= b_m[b_mid].x || ty < 0 || ty >= b_m[b_mid].y) continue;
                                for (int k = 0; k < b_m[b_mid].psize; k++) {
                                    if (p[k].x == tx && p[k].y == ty) {
                                        int dmg = roll("4d3", 1, b_m[b_mid].rng);
                                        pHP(p, b_m, k, b_mid, 0, dmg);
                                        p[k].buff_check[36]++; 
                                        *wofs << L"(T" << b_m[b_mid].time << L")" << e[I].name << L"用迫擊炮對" << p[k].name << L"造成" << dmg << L"點傷害並施加震懾" << endl;
                                    }
                                }
                                for (int k = 0; k < b_m[b_mid].nsize; k++) {
                                    if (b_n[k].x == tx && b_n[k].y == ty) {
                                        int dmg = roll("4d3", 1, b_m[b_mid].rng);
                                        nHP(b_n, b_m, k, b_mid, 0, dmg);
                                        b_n[k].buff_check[36]++;
                                        *wofs << L"(T" << b_m[b_mid].time << L")" << e[I].name << L"用迫擊炮對" << b_n[k].name << L"造成" << dmg << L"點傷害並施加震懾" << endl;
                                    }
                                }
                                for (int k = 0; k < b_m[b_mid].esize; k++) {
                                    if (e[k].x == tx && e[k].y == ty && k != I) {
                                        int dmg = roll("4d3", 1, b_m[b_mid].rng);
                                        eHP(e, b_m, k, b_mid, 0, dmg);
                                        e[k].buff_check[36]++;
                                        *wofs << L"(T" << b_m[b_mid].time << L")" << e[I].name << L"用迫擊炮對" << e[k].name << L"造成" << dmg << L"點傷害並施加震懾" << endl;
                                    }
                                }
                            }
                        }
                                ...
                                for (int sy = -1; sy < 2; sy++) {
                                    for (int k=0; k < b_m[b_mid].psize; k++) {
                                        if (p[k].x == e[I].target_x + sx && p[k].y == e[I].target_y + sy && te[e[I].target_x + sx][e[I].target_y + sy].type == 0) {
                                            int ro = roll("2d4", 1, b_m[b_mid].rng);
                                            if (p[k].buff_check[14] == 0) {
                                                if (p[k].dex + p[k].EDV >= roll("", 2, b_m[b_mid].rng)) {
                                                    ro /= 2;
                                                }
                                            }
                                ...
                                for (int sy = -5; sy <= 5; sy++) {          
                                    ...
                                    for (int k = 0; k < b_m[b_mid].psize; k++) {
                                            if (p[k].x == e[I].x + sx && p[k].y == e[I].y + sy ) {
                                                int ro = roll("3d3", 1, b_m[b_mid].rng);
                                                if (p[k].buff_check[14] == 0) {
                                                    if (p[k].dex + p[k].EDV >= roll("", 2, b_m[b_mid].rng)) {
                                                        ro /= 2;
                                                    }
                                    ...
                                    for (int k = 0; k < b_m[b_mid].nsize; k++) {
                                        if (b_n[k].x == e[I].x + sx && b_n[k].y == e[I].y + sy && te[e[I].x + sx][e[I].y + sy].type == 0) {
                                            int ro = roll("3d3", 1, b_m[b_mid].rng);
                                            if (b_n[k].dex  >= roll("", 2, b_m[b_mid].rng)) {
                                                ro /= 2;
                                            }
            ...
            if (e[id].buff_check[36] > 0) {
                e[id].buff_check[36]--;
                if (roll("", 2, b_m[b_mid].rng) > e[id].con) {
                    e[id].act--;
                }
            }
    }
    ...
    if (chose == "t") {
        if (b_m[b_mid].type == 1) {
            ...
            for (int J = 0; J < b_m[b_mid].y; J++) {
                if (te[I][J].flare == 1) {
                    te[I][J].flare_time--;
                    if (te[I][J].flare_time == 0) {
                        te[I][J].flare = 0;
                    }
                    else if (b_m[b_mid].type == 1) {
                        for (int fi = -5; fi <= 5; fi++) {
                            for (int fj = -5; fj <= 5; fj++) {
                                if ((te[I + fi][J + fj].dark == 1 || te[I + fi][J + fj].dark == 2) && (abs(fi) + abs(fj)) <= 5) {
                                    te[I + fi][J + fj].dark = 3;
                                }
                            }
                        }
            ...
            for (int I = 0; I < b_m[b_mid].esize; I++) {
                if (e[I].buff_check[39] != 0) {
                    if (e[I].pose == 1) {
                        for (int J = 0; J < e[I].w; J++) {
                            for (int K = 0; K < 6; K++) {
                                for (int L = 0; L <= K; L++) {
                                int tx = e[I].x + J - K+L;
								int ty = e[I].y + K + e[I].h - 1;
                                if (te[tx][ty].dark == 1||te[tx][ty].dark == 2) {
                                    te[tx][ty].dark = 3;
                                }
                                tx = e[I].x + J + K-L;
                                if (te[tx][ty].dark == 1 || te[tx][ty].dark == 2) {
...
void readmapjson(Map* m,int m_id, const char* filename) {
    ...
    if (reader.parse(in, root)) {
        ...
        for (unsigned int k = 0; k < root["layers"].size(); k++) {
            const Json::Value& props = root["layers"][k]["properties"];
            if (props.empty()) continue;  
            bool isBlock = false;
            int  seValue = 0;
            for (unsigned int p = 0; p < props.size(); p++) {
                const string name = props[p]["name"].asString();
                if (name == "block") {
                    isBlock = props[p]["value"].asBool();
                }
                else if (name == "SE") {
                    seValue = props[p]["value"].asInt();
                }
            }
            const Json::Value& data = root["layers"][k]["data"];
...
void  m_set(Map* m, npc* n, player* p,e_npc *e_n ,flag *f,BOX *Box,int m_id,int b_id);
void readeventjson(player* p, npc* n, flag* f, Map* m, BOX* Box, task* tk, m_flag* m_f, item* it, stone* st, const char* filename, int& m_id, int b_id) {
    ...
    settextstyle(30, 0, _T("Noto Sans TC"));
    ...
    loadimage(&pf0, L"./Game/picture/player0f.png", 0, 0, false);
    loadimage(&pf1, L"./Game/picture/player1f.png", 0, 0, false);
    loadimage(&pf2, L"./Game/picture/player2f.png", 0, 0, false);
    wstring mm, tt, ss, MM;
    Json::Reader reader;
    Json::Value root;
    ifstream in(filename, ios::binary);
    ...
    if (reader.parse(in, root)) {        
        ...
        for (int k = 0; k < root["talk"].size(); k++) {
            ...
            transparentimage(NULL, 0, 676, &t_block);
            if (root["talk"][k]["npc"].asInt() != -1) {
                settextstyle(50, 0, _T("Noto Sans TC"));
                ...
                if (root["talk"][k]["npc"].asInt() != 0) {
                    ...
                    transparentimage(NULL, 125, 780, &p1, 0xFF55FF);
                }
                ...
                drawtext(path, &t, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
                int fid = root["talk"][k]["face"].asInt();
                int pid = root["talk"][k]["player"].asInt();
                if (pid==0) {
                    transparentimageA(NULL, 125, 780, &pf0, fid%4*144, fid/4*144, 144, 144);
                }
                else if (pid == 1) {
                    transparentimageA(NULL,125, 780, &pf1, fid % 4 * 144, fid / 4 * 144, 144, 144);
                }
                else if (pid == 2) {
                    transparentimageA(NULL, 125, 780, &pf2, fid % 4 * 144, fid / 4 * 144, 144, 144);
                }
            }        
        settextcolor(WHITE);
        settextstyle(38, 0, _T("Noto Sans TC"));
        while (1) {
            start_time = clock();
            int Bu = F / 10;
            BeginBatchDraw();
            if (root.isMember("mType")) {
                if (root["mType"].asInt() == 1 && F % 129 == 0) {
                    ...
                    if (ep == 0) {
                        if (uj != 1 && uK == 0) {
                            if (uk % 3 == 1) {
                                transparentimage(NULL, 528, 528 - 96, &ma1, 0xFF55FF, 0, 96 * 2, 96, 96);
                            }
                            else if (uk % 3 == 2) {
                                transparentimage(NULL, 528, 528 - 96, &ma1, 0xFF55FF, 96 * 2, 96 * 2, 96, 96);
                            }
                            else if (uk % 3 == 0) {
                                transparentimage(NULL, 528, 528 - 96, &ma1, 0xFF55FF, 96, 96 * 2, 96, 96);
                            }
                        }
                        else {
                            if (uK % 3 == 1) {
                                transparentimage(NULL, 528, 528 - 96, &ma1, 0xFF55FF, 0, 0, 96, 96);
                            }
                            else if (uK % 3 == 0) {
                                transparentimage(NULL, 528, 528 - 96, &ma1, 0xFF55FF, 96 * 2, 0, 96, 96);
                            }
                            else if (uK % 3 == 2) {
                                transparentimage(NULL, 528, 528 - 96, &ma1, 0xFF55FF, 96, 0, 96, 96);
                    ...
                    if (k == 5 && pa != 1 && ei == 0) {
                        transparentimage(NULL, 48 * 18, 48 * 7, &ma1, 0xFF55FF, 96, 96, 96, 96);
                    }
                    if (k == 4 && w >= s.size() - 1 && ubs > -1 && pa == 0) {
                        pa = 1;
                        ubs = 16 * 3;
                    ...
                    if (pa == 1 && k == 5) {
                        ...
                        if (ei < 15) {
                            if (ei % 3 == 0) {
                                transparentimage(NULL, 48 * 18 - ei * 16, 48 * 7, &ma1, 0xFF55FF, 96, 96, 96, 96);
                            }
                            else if (ei % 3 == 1) {
                                transparentimage(NULL, 48 * 18 - ei * 16, 48 * 7, &ma1, 0xFF55FF, 0, 96, 96, 96);
                            }
                            else if (ei % 3 == 2) {
                                transparentimage(NULL, 48 * 18 - ei * 16, 48 * 7, &ma1, 0xFF55FF, 96 * 2, 96, 96, 96);
                            }
                            ei++;
                        }
                        if (ubs >= 3 * 13 - 1) {
            ...
            int ku = 0;
            if (s[w] == '#') {
                getimage(&gt, 0, 0, 1296, 695);
                int R=int(s[++w])-'0',cType=-1,cHtype=-1;
                IMAGE q1,q2;
                ExMessage em;
                W = 0;
                loadimage(&q1, L"./Game/picture/btn_select_normal.png", 0, 0, false);
                loadimage(&q2, L"./Game/picture/btn_select_hover.png", 0, 0, false);         
                BeginBatchDraw();
                ...
                while (ku == 0) {
                    while (peekmessage(&em, EM_MOUSE)) {
                        ...
                        if (em.message == WM_LBUTTONDOWN) {
                            for (int I = 0; I < R; I++) {
                                if (mouseX > 250 && mouseX < 1050 &&mouseY > 196 + I * 150 &&mouseY < 246 + I * 150) {
                                    SEplay("button05",1 );
                    ...
                    for (int I = 0; I < R; I++) {
                        W = 0;
                        bool hover =mouseX > 250 && mouseX < 1050 &&mouseY > 196 + I * 150 &&mouseY < 246 + I * 150;
                        transparentimage(NULL, 250, 196 + I * 150,hover ? &q2 : &q1);
                        if (hover) {
                            cType = I;
                        }
                        if (cType != -1 && cType != cHtype) {
                            cHtype = I;
                            SEplay("button05", 2);
                        }
                        while (q < s.size()) {
                            ...
                            if (s[q] == '^') {
                                ...
                                tt += s[q];
                                LPCTSTR path = tt.c_str();
                                outtextxy(30 * W + 300, 196 + I * 150 + 5, path);
                                W++;
                            }
                            q++;
                        }
            ...
            if (w == s.size() && tE == 0) {
                getimage(&gt, 0, 696, 1296, 960);
                tE = 1;
            }
            if (w == s.size() && isReady) {
                putimage(0,696,&gt);          
                if (talkEnd%30 != 0) {
                     transparentimage(NULL, 30 * W + 300, 765 + K * 50+5, &tri1);
                    talkEnd ++;
                }
                else {
                    transparentimage(NULL, 30 * W + 300, 765 + K * 50 + 5, &tri);
                    talkEnd = 1;
                }
                FlushBatchDraw();
                flushmessage(-1);
        ...
        for (int k = 0; k < root["talk"].size(); k++) {
            ...
            if (root["talk"][k]["npc"].asInt() != -1) {
                ...
                if (root["talk"][k]["npc"].asInt() != 0) {
                    ...
                    LPCTSTR path = mm.c_str();
                    loadimage(&p1, path, 0, 0, false);
                  transparentimage(NULL, 60, 750, &p1, 0xFF55FF);
                }
            }
            else {
                settextstyle(50, 0, _T("Noto Sans TC"));
            ...
            for (j = 0; j < root["see"].size();j++) {  
                ...
                for (int w = 0; w < s.size(); w++) {
                    ...
                    if (s[w] == '^') {
                        ...
                        settextstyle(38, 0, _T("Noto Sans TC"),0,0,0,false, false,bo);
                        bo = !bo;
                    }
                    else {
                        tt += s[w];
...
void e_put(enemy *e,b_map *b_m, terrain(*te)[50],int esize,int b_mid) {
    ...
    for (int I = 0; I < esize; I++) {
        if (e[I].h == 1&&e[I].w==1) {
            ...
            mm = L"./Game/picture/enemy" + to_wstring(e[I].type) + L".png";
            LPCTSTR path = mm.c_str();
            loadimage(&b0, path, 0, 0, false);
            loadimage(&hpB, L"./Game/picture/hp.png", 0, 0, false);
            if (e[I].pose == 1) {
                transparentimage(NULL, e[I].x * 48 - b_m[b_mid].ox, e[I].y * 48 - b_m[b_mid].oy - 16, &b0, 0xFF55FF, 64 + 8, 0, 48, 64);
            }
            else if (e[I].pose == 2) {
                transparentimage(NULL, e[I].x * 48 - b_m[b_mid].ox, e[I].y * 48 - b_m[b_mid].oy - 16, &b0, 0xFF55FF, 64 + 16, 64, 48, 64);

            }
            else if (e[I].pose == 3) {
                transparentimage(NULL, e[I].x * 48 - b_m[b_mid].ox, e[I].y * 48 - b_m[b_mid].oy - 16, &b0, 0xFF55FF, 64 + 0, 128, 48, 64);

            }
            else if (e[I].pose == 4) {
                transparentimage(NULL, e[I].x * 48 - b_m[b_mid].ox, e[I].y * 48 - b_m[b_mid].oy - 16, &b0, 0xFF55FF, 64 + 8, 192, 48, 64);
            ...
            transparentimage(NULL, e[I].x * 48 - b_m[b_mid].ox, e[I].y * 48 - b_m[b_mid].oy + (48 - ((e[I].hp * 48) / e[I].mhp)), &hpB, 0xFF55FF, 0, 48 - ((e[I].hp * 48) / e[I].mhp), 48, ((e[I].hp * 48) / e[I].mhp));
                        ...
                        mm = L"./Game/picture/enemy" + to_wstring(e[I].type) + L"_1.png";
                        loadimage(&b1, mm.c_str(), 0, 0, false);
                        mm = L"./Game/picture/enemy" + to_wstring(e[I].type) + L"_2.png";
                        loadimage(&b2, mm.c_str(), 0, 0, false);
                        loadimage(&hpB, L"./Game/picture/hp.png", 0, 0, false);
            for (int W = 0; W < e[I].w; W++) {
                for (int H = 0; H < e[I].h; H++) {
                    if ((e[I].x+W) * 48 - b_m[b_mid].ox < 960 && (e[I].y+H) * 48 - b_m[b_mid].oy < 720 && (te[e[I].x+W][e[I].y+H].dark == 0 || te[e[I].x+W][e[I].y+H].dark == 2 || te[e[I].x+W][e[I].y+H].dark == 3)) {
                        tur = 1;                       
                    }
                }
            }
            if (tur == 1) {
                for (int W = 0; W < e[I].w; W++) {
                    for (int H = 0; H < e[I].h; H++) {
                        if (e[I].pose == 1) {
                            transparentimage(NULL, (e[I].x + W) * 48 - b_m[b_mid].ox, (e[I].y + H) * 48 - b_m[b_mid].oy , &b2, 0xFF55FF, e[I].w * 48 + W * 48, 0 + H * 48, 48, 48);
                        }
                        else if (e[I].pose == 2) {
                            transparentimage(NULL, (e[I].x + W) * 48 - b_m[b_mid].ox, (e[I].y + H) * 48 - b_m[b_mid].oy , &b1, 0xFF55FF, e[I].w * 48 + W * 48, 0 + H * 48, 48, 48);
                        }
                        else if (e[I].pose == 3) {
                            transparentimage(NULL, (e[I].x + W) * 48 - b_m[b_mid].ox, (e[I].y + H) * 48 - b_m[b_mid].oy , &b1, 0xFF55FF, e[I].w * 48 + W * 48, e[I].h * 48 + H * 48, 48, 48);
                        }
                        else if (e[I].pose == 4) {
                            transparentimage(NULL, (e[I].x + W) * 48 - b_m[b_mid].ox, (e[I].y + H) * 48 - b_m[b_mid].oy , &b2, 0xFF55FF, e[I].w * 48 + W * 48, e[I].h * 48 + H * 48, 48, 48);
                        }
                    }
                }
            transparentimage(NULL, e[I].x * 48 - b_m[b_mid].ox, e[I].y * 48 - b_m[b_mid].oy + (48 - ((e[I].hp * 48) / e[I].mhp)), &hpB, 0xFF55FF, 0, 48 - ((e[I].hp * 48) / e[I].mhp), 48, ((e[I].hp * 48) / e[I].mhp));
        ...
        if (I != id) {
            if (e[I].x * 48 - b_m[b_mid].ox < 960 && e[I].y * 48 - b_m[b_mid].oy - 16 - 16 < 720 && (te[e[I].x][e[I].y].dark == 0 || te[e[I].x][e[I].y].dark == 2|| te[e[I].x][e[I].y].dark == 3)) {
                ...
                if (e[I].pose == 1) {
                    ...
                    for (int W = 0; W < e[I].w; W++) {
                        for (int H = 0; H < e[I].h; H++) {
                            if ((e[I].x + W) * 48 - b_m[b_mid].ox < 960 && (e[I].y + H) * 48 - b_m[b_mid].oy - 16 - 16 < 720 && (te[e[I].x + W][e[I].y + H].dark == 0 || te[e[I].x + W][e[I].y + H].dark == 2 || te[e[I].x + W][e[I].y + H].dark == 3)) {
                                tur = 1;
                            }
                        }
                    }
...
void maps(player *p,enemy *e,b_map *b_m, arms *ar,terrain(*te)[50],int b_mid) {
    ...
    IMAGE te2,io,te3,io1,te4,Pit,Flare;          
    ...
    loadimage(&Pit, L"./Game/picture/pit.png", 0, 0, false);
    loadimage(&Flare, L"./Game/picture/nc202351_光の玉_光源.png", 0, 0, false);
    for (int I = 0; I < b_m[b_mid].esize; I++) {
    ...
    for (int I = b_m[b_mid].ox/48; I < b_m[b_mid].ox / 48+20; I++) {
        for (int J = b_m[b_mid].oy / 48; J < b_m[b_mid].oy / 48+15; J++) {
            if (te[I][J].pit == 1) {
                transparentimage(NULL, 48 * I - b_m[b_mid].ox, 48 * J - b_m[b_mid].oy, &Pit);
            }
            if (te[I][J].danger == 1) {
                transparentimage(NULL, 48 * I - b_m[b_mid].ox, 48 * J - b_m[b_mid].oy, &io);
            }
            if (te[I][J].fire == 1) {
    ...
    for (int J = b_m[b_mid].oy / 48; J < b_m[b_mid].oy / 48 + 15; J++) {
    if (te[I][J].flare == 1) {
        transparentimage(NULL, 48 * I - b_m[b_mid].ox - 292, 48 * J - b_m[b_mid].oy - 226, &Flare);
    }
        }
    }
}
...
void De(b_map *b_m,terrain(*te)[50],enemy *e,player *p,int id,int b_mid) {
    if (e[id].hp <= 0) {
        if (e[id].w == 1 && e[id].y == 1) {
            te[e[id].x][e[id].y].mA -= 10000;
            te[e[id].x][e[id].y].mB -= 10000;
            int x = e[id].x, y = e[id].y;
            if (x + 1 < b_m[b_mid].x && te[x + 1][y].type == 0) {
...
void AE(int ar_id,int id,int P_id,int b_mid,int psize,int esize,int bu_id,player *p,enemy *e,b_map *b_m,int type){
    IMAGE ae;
    wstring vos;
    if (type == 0) {
    if (ar_id == 0 || ar_id == 5) {                
...
void dmg(wofstream *wofs, terrain(*te)[50],player *p,enemy *e,arms *ar,b_map *b_m,int P_id,int id,int b_mid,int ar_id,int type,int x,int y) {
    wstring mm;
    int AC;
    if (type == 0) {
        if (e[id].buff_check[20] == 1) {
        ...
        int ATK = roll("", 2, b_m[b_mid].rng) + (p[P_id].dex - 10) / 2 + roll(ar[ar_id].hit, 1, b_m[b_mid].rng);
        if (ar[ar_id].type[0] == 'r' && p[P_id].buff_check[25] == 1) {
            AC = 0;
            p[P_id].buff_check[25] = 0;
            p[P_id].buff_check[24] = 1;
        ...
        if (ATK > AC) {
            int dmg = roll(ar[ar_id].dmg, 1, b_m[b_mid].rng);
            if (ar[ar_id].type[0] == 'm') {
                dmg += (p[P_id].str - 10) / 5;
            }
            if (dmg >= e[id].buff_check[7] + e[id].buff_check[10]) {
            ...
            *wofs << L"(T" << b_m[b_mid].time << L")" << p[P_id].name << L"用" << ar[ar_id].name << L"命中" << e[id].name << L"造成" << dmg << L"點傷害(" << ATK << ">" << AC << ")" << endl;
            EndBatchDraw();
            if (ar[ar_id].type[0] == 'r') {
                if (ar_id == p[P_id].arms_id_1 && p[P_id].arms_b_1 > 0) {
                    p[P_id].arms_b_1--;
                }
                else {
                    p[P_id].arms_b_2--;
            ...
            *wofs << L"(T" << b_m[b_mid].time << L")" << p[P_id].name << L"用" << ar[ar_id].name << L"攻擊" << e[id].name << L"未能命中(" << ATK << "<=" << AC << ")" << endl;
            ...
            settextcolor(RGB(255, 0, 0));
            settextstyle(25, 0, _T("Taipei Sans TC Beta"));
            outtextxy(x * 48 - b_m[b_mid].ox, y * 48 - b_m[b_mid].oy - 40, mm.c_str());
            settextstyle(18, 0, _T("Taipei Sans TC Beta"));
            settextcolor(RGB(0, 0, 0));
            Sleep(1000);
        }
            ...
            for (i = 0; i < b_m[b_mid].x; i++) {
                ...
                *wofs << L"(T" << b_m[b_mid].time << L")" << p[P_id].name << L"將" << e[id].name << L"選為目標" << endl;
                ...
                if (rx + ry <= range) {
                    ...
                    getimage(&BGBuffer, 0, 0, 960, 720);
                    AE(ar_id, id, P_id, b_mid, psize, esize, bu_id, p, e, b_m,0);
                    if (ATK > AC) {
                         BeginBatchDraw();
                        ...
                        b_nput(b_n, b_m, b_m[b_mid].nsize, b_mid);
                        p_put(p, b_m, psize, b_mid);
                        dmg = roll(ar[ar_id].dmg, 1, b_m[b_mid].rng);
                        if (ar[ar_id].type[0] == 'm') {
                            dmg += (p[P_id].str - 10) / 5;
                        }
                        if (p[P_id].s_check[15] == 1) {
                        ...
                        if (p[P_id].buff_check[21] > 0) {
                            dmg += (p[P_id].wis - 5) / 2;
                            string mm1 = to_string((p[P_id].mhp - p[P_id].hp) / 5);
                            p[P_id].hp += (p[P_id].mhp - p[P_id].hp) / 5;
                            wstring path = L"./Game/picture/ls.png";
                            IMAGE da;
                            loadimage(&da, path.c_str());
                            int totalWidth = mm1.size() * 20;
                            for (int J = 0; J < mm1.size(); J++) {
                                transparentimageA(NULL, p[P_id].x * 48 - b_m[b_mid].ox + 24 - totalWidth / 2, p[P_id].y * 48 - b_m[b_mid].oy - 40, &da, int(mm1[J] - '0') * 20, 0, 20, 28);
                            }
                        }      
                        eHP(e, b_m,id,b_mid, 0, dmg);
                        *wofs << L"(T" << b_m[b_mid].time << L")" << p[P_id].name << L"用" << ar[ar_id].name << L"命中" <<  e[id].name<< L"造成" << dmg << L"點傷害(" << ATK << ">" << AC << ")" << endl;
                    ...
                    for (i = 0; i < b_m[b_mid].psize;i++) {
                        if (p[i].buff_check[28] > 0) {                      
                                    ...
                                    if (abs(p[i].x - e[id].x) + abs(p[i].y - e[id].y) <= range && ar_id != -1) {
                                        int AC = roll("", 2, b_m[b_mid].rng) + (e[id].dex - 10) / 2;
                                        if (e[id].buff_check[20] == 1) {
                                            AC = 0;
                                        }
                                        if (e[id].buff_check[27] > 0) {
                                         ...
                                         int ATK = roll("", 2, b_m[b_mid].rng) + (p[i].dex - 10) / 2 + roll(ar[ar_id].hit, 1, b_m[b_mid].rng);
                                         getimage(&BGBuffer, 0, 0, 960, 720);
                                         AE(ar_id, id, P_id, b_mid, psize, esize, bu_id, p, e, b_m,0);
                                        /*ae*/
                                        if (ATK > AC) {
                                ...
                                for (int I = -1; I <= 1; I++) {
                                    for (int J = -1; J <= 1; J++) {
                                        for (int pi=0; pi < b_m[b_mid].psize; pi++) {
                                            ...
                                            int DMG = roll("2d3",1, b_m[b_mid].rng);
            ...
            for (int I = 0; I < b_m[b_mid].psize; I++) {
                if (p[I].buff_check[22] > 0) {
                    ...
                    if (abs(p[I].x - e[id].x) + abs(p[I].y - e[id].y) <= range && ar_id != -1) {
                        ...
                        int ATK;
                        if (ar[ar_id].type[0] == 'r') {
                            ATK = roll("", 2, b_m[b_mid].rng) + (p[I].dex - 10) / 2 + roll(ar[ar_id].hit, 1, b_m[b_mid].rng);
                        }
                        else if (ar[ar_id].type[0] == 'm') {
                            ATK = roll("", 2, b_m[b_mid].rng) + (p[I].str - 10) / 2 + roll(ar[ar_id].hit, 1, b_m[b_mid].rng);;
                        }
                        wstring vos;
                        IMAGE ae;
                        if (ar_id == 0 || ar_id == 5) {
...
void e_walk(wofstream *wofs,enemy* e, player* p,b_npc *b_n, b_map* b_m,arms *ar, terrain(*te)[50], int id, int P_id, int esize, int psize, int b_mid, int bu_id, int b_nid, int target,int type) {
    ...
    while (1) {
        ...
        for (int I = 0; I < b_m[b_mid].psize; I++) {
            if (p[I].buff_check[22] > 0&&sb==0) {
                ...
                if (abs(p[I].x - xbox) + abs(p[I].y - ybox) <= range && ar_id != -1) {
                    sb = 1;
                    dmg(wofs, te, p, e, ar, b_m, I, id, b_mid, ar_id, 0, xbox, ybox);
                    p[I].buff_check[22]--;
                }
            }
        }
        ...
        *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name << L"從(" << e[id].x << L"," << e[id].y << L")到(" << xbox << L"," << ybox << L")" << endl;
        ...
        xbox = e[id].x; ybox = e[id].y;
        wstring mm;
        wstring mm1 = std::to_wstring(e[id].type);
        mm1 = L"./Game/picture/enemy" + mm1 + L"_1.png";
        wstring mm2 = std::to_wstring(e[id].type);
        mm2 = L"./Game/picture/enemy" + mm2 + L"_2.png";
        IMAGE enemy1;
        loadimage(&enemy1, mm1.c_str(), 0, 0, false);
        IMAGE enemy2;
        loadimage(&enemy2, mm2.c_str(), 0, 0, false);
        b_camera(b_m, xbox, ybox, b_mid);
        int MB,P=e[id].pose,rx,ry, eh=e[id].h, ew=e[id].w;
        *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name << L"從(" << e[id].x << L"," << e[id].y << L")到(";
        int x = e[id].x, y = e[id].y;
        while (1) {
            if (box[w] / 10 ==2|| box[w] / 10 == 8) {
                rx = xbox - ((eh - ew) / 2 + (eh - ew) % 2);
                ry =ybox + ((eh -ew) / 2 + (eh - ew) % 2);
            }
            else if (box[w] / 10 == 4 || box[w] / 10 == 6) {
                rx = xbox + ((ew - eh) / 2 + (ew - eh) % 2);
                ry = ybox - ((ew - eh) / 2 + (ew - eh) % 2);
            }
            if (box[w] == 2) {
                int Mmove=0;
                for (int I = 0;I < ew;I++) {
                ...
                if (box[w] / 10 == 4) {
                    ...
                    for (int I = 0;I < eh;I++) {
                        for (int J = 0;J < ew;J++) {
                            if (ry + J < ybox || ry + J >= ybox + eh) {
                                if (te[rx + I][ry + J].mB > Mmove) {
                                    Mmove = te[rx + I][ry + J].mB;
                                }
                                if (te[rx + I][ry + J].fire == 1) {
                                    if (e[id].buff_check[11] > 0) {
            ...
            if (box[w] == 2) {
                ...
                for (k = 0; k <3; k ++) {
                    ...
                    if (xbox * 48 + k*16 - b_m[b_mid].ox < 960 && ybox * 48 - b_m[b_mid].oy  < 720) {
                        transparentimage(NULL, xbox * 48 + 16*k - b_m[b_mid].ox, ybox * 48 - b_m[b_mid].oy -(k%2), &enemy1, 0xFF55FF, 48 * ew * (k % 2 - (k / 2) + 1), eh * 48, ew * 48, eh * 48);
                    }
                    ui(p, e, b_m, P_id, esize, psize, b_mid, bu_id);
                    EndBatchDraw();
                    Sleep(30);
                ...
                if (P == 1) {
                    ...
                    transparentimage(NULL, xbox * 48 - b_m[b_mid].ox, ybox * 48 - b_m[b_mid].oy, &enemy1, 0xFF55FF, ew * 48  , 0, 48*ew, 48*eh);
                }
                else if (P == 3) {
                    transparentimage(NULL, xbox * 48 - b_m[b_mid].ox, ybox * 48 - b_m[b_mid].oy, &enemy1, 0xFF55FF, ew * 48, eh*48, 48 * ew, 48 * eh);
                }
                else if (P == 4) {
            ...
            if (te[xbox][ybox].trap == 2) {
                ...
                *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name << "觸發狩獵陷阱" << endl;
                ...
                if (e[id].hp <= 0) {/*大型清空(未完成*/
                    ...
                    te[xbox][ybox].trap = 0;
                    *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name << "破除狩獵陷阱" << endl;
                }
            }
            e[id].x = xbox; e[id].y = ybox;
            e[id].h = eh; e[id].w = ew;
...
void acts( player *p,enemy *e,b_npc *b_n,b_map *b_m,arms *ar,buff *bu, terrain(*te)[50],string& chose,int b_mid, int P_id,int esize,int id,int psize,int bu_id,int b_nid) {
    ...
    {               
        ...
        if ((GetAsyncKeyState(VK_UP) & 0x8000)) {
            ...
            if (b_m[b_mid].debug_mode) {
                b_m[b_mid].debug_mode = false;
            }
            else {
                b_m[b_mid].debug_mode = true;
            }
        }
        else {
                if (m.message == WM_MOUSEMOVE) {                    
                    for (i = 0; i < esize; i++) {
                        if (e[i].w == 1 && e[i].h == 1) {
                            ...
                            for (int W = 0; W < e[i].w; W++) {
                                for (int H = 0; H < e[i].h; H++) {
                                    if (m.x >= (e[i].x+W) * 48 - b_m[b_mid].ox && m.x <= (e[i].x+W) * 48 + 48 - b_m[b_mid].ox && m.y >= (e[i].y+H) * 48 - b_m[b_mid].oy && m.y <= (e[i].y+H) * 48 + 48 - b_m[b_mid].oy && Flag != i && m.x < 960 && m.y < 720) {
                                        for (int W = 0; W < e[i].w; W++) {
                                            for (int H = 0; H < e[i].h; H++) {
                                                te[e[i].x + W][e[i].y + H].mB -= 10000;
                                                te[e[i].x + W][e[i].y + H].enemy = 0;
                                            }
                                        }
                                        BeginBatchDraw();
                                        maps(p, e, b_m, ar, te, b_mid);/*地圖繪製*/
                                        ...
                                        while (q.size() > 0) {  
                                            ...
                                            if (q.front().po == 1) {
                                                ...
                                                for (int I = 0; I < q.front().w; I++) {
                                                    if (te[q.front().x + I][q.front().y+q.front().h].player == 1 || te[q.front().x + I][q.front().y + q.front().h].enemy== 1|| te[q.front().x + I][q.front().y + q.front().h].npc == 1|| te[q.front().x + I][q.front().y + q.front().h].type == 1|| te[q.front().x + I][q.front().y + q.front().h].mB > q.front().mo|| q.front().y + q.front().h>b_m[b_mid].y) {
                                                        ubp1 = 1;
                                                        te[26][29].mB = te[26][29].mB;
                                                    }
                                                    if (mo1 < te[q.front().x + I][q.front().y + q.front().h].mB) {
                                                        mo1 = te[q.front().x + I][q.front().y + q.front().h].mB;
                                                    }
                                                ...
                                                for (int I = 0; I < q.front().w;I++) {
                                                    if (te[q.front().x + I][q.front().y - 1].player == 1 || te[q.front().x + I][q.front().y -1].enemy == 1 || te[q.front().x + I][q.front().y -1].npc == 1 || te[q.front().x + I][q.front().y-1].type == 1 || te[q.front().x + I][q.front().y -1].mB > q.front().mo||q.front().y-1<0 ) {
                                                        ubp4 = 1;                                                        
                                                    }
                                                    if (mo4 < te[q.front().x + I][q.front().y - 1].mB) {
                                                        mo4 = te[q.front().x + I][q.front().y - 1].mB;
                                                ...
                                                if (ubp1 == 0) {            
                                                    ...
                                                    else if(q.front().mo - mo1 - 2>=0){
                                                        data.x = q.front().x;
                                                        data.y = q.front().y + 1;
                                                        data.po = 4;
                                                        data.w = q.front().w;
                                                        data.h = q.front().h;
                                                        data.mo = q.front().mo - mo1-2;
                                                        q.push(data);
                                                    }
                                                }
                                                if (ubp2 == 0) {
                                                ...
                                                if (ubp4 == 0) {
                                                    ...
                                                    data.w = q.front().w;
                                                    data.h = q.front().h;
                                                    data.mo = q.front().mo - mo4;
                                                    q.push(data);
                                                    }
                                                    else if (q.front().mo - mo4 - 2 >= 0) {
                                                        data.x = q.front().x;
                                                        data.y = q.front().y - 1;
                                                        data.po = 1;
                                                        data.w = q.front().w;
                                                        data.h = q.front().h;
                                                        data.mo = q.front().mo - mo4-2;
                                                        q.push(data);
                                                    }
                                                }                             
                                            }
                                                ...
                                                for (int J = 0; J < q.front().h; J++) {
                                                    if (te[q.front().x - 1][q.front().y + J].player == 1 || te[q.front().x - 1][q.front().y + J].enemy == 1 || te[q.front().x - 1][q.front().y + J].npc == 1 || te[q.front().x - 1][q.front().y + J].type == 1 || te[q.front().x - 1][q.front().y + J].mB > q.front().mo||q.front().x-1<0) {
                                                        ubp2 = 1;
                                                    }
                                                    if (mo2 < te[q.front().x - 1][q.front().y + J].mB) {
                                                        mo2 = te[q.front().x - 1][q.front().y + J].mB;
                                                ...
                                                for (int J = 0; J < q.front().h; J++) {
                                                    if (te[q.front().x + q.front().w][q.front().y + J].player == 1 || te[q.front().x + q.front().w][q.front().y + J].enemy == 1 || te[q.front().x + q.front().w][q.front().y + J].npc == 1 || te[q.front().x + q.front().w][q.front().y + J].type == 1 || te[q.front().x + q.front().w][q.front().y + J].mB > q.front().mo||q.front().x+q.front().w>=b_m[b_mid].x ) {
                                                        ubp3 = 1;
                                                    }
                                                    if (mo3 < te[q.front().x + q.front().w][q.front().y + J].mB) {
                                                        mo3 = te[q.front().x + q.front().w][q.front().y + J].mB;
                                                ...
                                                if (ubp2 == 0) {
                                                    ...
                                                    else if (q.front().mo-mo2-2>=0) {
                                                        data.x = q.front().x - 1;
                                                        data.y = q.front().y;
                                                        data.po = 3;
                                                        data.w = q.front().w;
                                                        data.h = q.front().h;
                                                        data.mo = q.front().mo - mo2-2;
                                                        q.push(data);
                                                    }
                                                }
                                                if (ubp3 == 0) {
                                                    ...
                                                    else if (q.front().mo - mo3 - 2 >= 0) {
                                                        data.x = q.front().x + 1;
                                                        data.y = q.front().y;
                                                        data.po = 2;
                                                        data.w = q.front().w;
                                                        data.h = q.front().h;
                                                        data.mo = q.front().mo - mo3-2;
                                                        q.push(data);
                                                    }
                                                }
                                                if (ubp4 == 0) {
                                        ...
                                        for (int W = 0; W < e[i].w; W++) {
                                            for (int H = 0; H < e[i].h; H++) {
                                                te[e[i].x + W][e[i].y + H].mB += 10000;
                                                te[e[i].x + W][e[i].y + H].enemy = 1;
                                            }
                                        }
                                        for (int I = 0; I < b_m[b_mid].x; I++) {
                                            for (int J = 0; J < b_m[b_mid].y; J++) {
                                                if (move[I][J]==1&&te[I][J].enemy == 0) {
                                                    transparentimage(NULL, 48 * I - b_m[b_mid].ox, 48 * J - b_m[b_mid].oy, &mb);
                                                }
                                                else if (dr[I][J] == 1&&te[I][J].enemy==0) {
                                                    transparentimage(NULL, 48 * I - b_m[b_mid].ox, 48 * J - b_m[b_mid].oy, &ab);
                                                }
                                            }
                                        }
                    ...
                    if (m.x<960&&m.y<720) {
                        ...
                        tew = L"友方移動損耗:"+to_wstring(te[b_m[b_mid].mx][b_m[b_mid].my].mA)+L" 敵方移動損耗:"+to_wstring(te[b_m[b_mid].mx][b_m[b_mid].my].mB);
                        outtextxy(970, 570 + ti * 40, tew.c_str());
                        ti++;
                        /*if (b_m[b_mid].debug_mode) {
...
void event(flag *f,b_flag *b_f,player *p,npc *n,m_flag *m_f,Map *m,BOX *Box,Exit *EX,enemy *e,e_npc *e_n,task *tk,shop *sp,item *it,stone *st,arms *ar,int &m_id,int &b_mid,int b_id) {
    ...
    if (f[0].check == 0) {
     ...
     if (f[20].check == 1) {
         ...
         tk[5].state = 1;
         EX[1].state = 0;
         m_set(m, n, p, e_n, f,Box, m_id, b_id);
         f[21].check = 1;
     }
     if (f[21].check == 1) {
...
void m_set(Map* m, npc* n, player* p,e_npc *e_n ,flag *f,BOX *Box,int m_id,int b_id) {
    ...
    mapP = boxx;
    if (f[21].check==1) {
        mm = L"./Game/map/siy.png";
        loadimage(&lightP, mm.c_str(), 0, 0, false);
    }
    else {
    mm = L"./Game/map/light" + to_wstring(m_id) + L".png";
    loadimage(&lightP, mm.c_str(), 0, 0, false);
    }
    i = 0;
    for (i = 0; i < b_id; i++) {
...
void m_check(player* p, Map* m, Exit* ex,flag *f,BOX *Box, int& m_id,int b_id,npc *n,e_npc *e_n) {
    string s = "";
    int uuu,u,k;
...
void menu_system() {
    ...
    loadimage(&tri1, L"./Game/picture/rarrow.png", 0, 0, false);
    loadimage(&tri2, L"./Game/picture/larrow.png", 0, 0, false);
    loadimage(&play, L"./Game/picture/play.png", 0, 0, false);
    while (1) {
        BeginBatchDraw();
        putimage(400, 0, &b1);    
        ...
        text = L"BGM";
        outtextxy(800-2, 50, text.c_str());
        text = to_wstring(voiceSize / 10);
        outtextxy(810, 80,text.c_str());
        transparentimage(NULL, 1200, 180, &tri2, 0xFF55FF);
        transparentimage(NULL, 425, 180, &tri1, 0xFF55FF);
        text = L"SE";
        outtextxy(800+8, 150, text.c_str());
        text = to_wstring(soundSize/10);
        outtextxy(810, 180, text.c_str());
        transparentimage(NULL, 1200, 280, &tri2, 0xFF55FF);
        transparentimage(NULL, 425, 280, &tri1, 0xFF55FF);
        text = L"VO";
        outtextxy(800+8, 250, text.c_str());
        transparentimage(NULL,800+48,250-4,&play);
        text = to_wstring(voSize / 10);
        outtextxy(810, 280, text.c_str());
        EndBatchDraw();
        ...
        if (em.x >= 1200 && em.x <= 1250 && em.y > 80 && em.y < 130 && voiceSize<1000 && em.lbutton) {
            voiceSize += 100;
            SEplay("決定ボタンを押す2", 1);
            vos = L"setaudio bgm volume to " + to_wstring(voiceSize);
            mciSendString(vos.c_str(), NULL, 0, NULL);
        ...
        if (em.x >= 1200 && em.x <= 1250 && em.y > 180 && em.y < 230 && soundSize < 1000 && em.lbutton) {
            soundSize += 100;
            SEplay("決定ボタンを押す2", 1);
        }
        if (em.x >= 425 && em.x <= 475 && em.y > 280 && em.y < 330 && voSize >0 && em.lbutton) {
            voSize -= 100;
            SEplay("決定ボタンを押す2", 1);
        }
        if (em.x >= 1200 && em.x <= 1250 && em.y > 280 && em.y < 330 && voSize < 1000 && em.lbutton) {
            voSize += 100;
            SEplay("決定ボタンを押す2", 1);
        }
        if (em.x >= 848 && em.x <= 848+36 && em.y > 250-4 && em.y < 250-4+36 && em.lbutton) {
            VOplay("syozyo8-arigatone", 1);
        }
        if (em.rbutton) {
...
void show(Map* m, player* p, enemy *e,e_npc *e_n,BOX *Box,int &m_id,int b_id, npc* n,flag *f,string g) {
    ...
    transparentimage(NULL, 0, 0, &lightP);
    FlushBatchDraw();
    /*       */
    if (tb - ta <25) {
        Sleep(25 - (tb - ta));
        ...
        if (b_m[b_mid].e_set[i] == 'x') {
            ...
            while (1) {
                if (b_m[b_mid].e_set[i] >= '0' && b_m[b_mid].e_set[i] <= '9') {
                    ...
                    e[k].s_range = e_t[id].s_range; e[k].e_range = e_t[id].e_range; e[k].h = e_t[id].h; e[k].w = e_t[id].w; e[k].Act = e_t[id].Act;
                    if (e[k].baid == 10) {
                        e[k].b_num_1 = 1;
                        e[k].b_num_2 = 1;
                    }
        ...
        if (b_m[b_mid].n_set[i] == 'x') {
            ...
            while (1) {
                if (b_m[b_mid].n_set[i] >= '0' && b_m[b_mid].n_set[i] <= '9') {
                    ...
                    b_n[k].name = b_nt[id].name; b_n[k].Act = b_nt[id].Act;
                    i--;
                    break;
                }
            }
...
void enemy_type_get(enemy *e,enemy_type *e_t, terrain(*te)[50],b_map *b_m,int b_mid,int id,int x,int y,int pose) {
    ...
    e[k].wis = e_t[id].wis; e[k].cha = e_t[id].cha; e[k].lv = e_t[id].lv; e[k].mhp = e_t[id].mhp; e[k].hp = e_t[id].hp;
    e[k].Move = e_t[id].Move; e[k].speed = e_t[id].speed; e[k].turn = 0; e[k].type = id; e[k].exp = e_t[id].exp;
    e[k].s_range = e_t[id].s_range; e[k].e_range = e_t[id].e_range;     e[k].pose = pose; e[k].Act = e_t[id].Act;
    if (pose == 2 || pose == 3) {
    ...
    if (y - 1 >= 0 && te[x][y - 1].type == 0) {
        ...
        for (int W = 0; W < e[k].w; W++) {
            for (int H = 0; H < e[k].h; H++) {
                te[x+W][y+H].mA += 10000;
                te[x+W][y+H].mB += 10000;
                te[x+W][y+H].enemy = 1;
                if (x+W+1==x+e[k].w && x + W + 1 < b_m[b_mid].x && te[x + W + 1][y + H].type == 0) {
                    te[x+W + 1][y+H].mA += 1;
                }
                if (x+W-1<x&&x+W - 1 >= 0 && te[x+W - 1][y+H].type == 0) {
                    te[x+W - 1][y+H].mA += 1;
                }
                if (y+H+1==y+e[k].h && y + H + 1 < b_m[b_mid].y && te[x + W][y + H + 1].type == 0) {
                    te[x+W][y +H+ 1].mA += 1;
                }
                if (y+H-1<y&&y+H - 1 >= 0 && te[x+W][y+H - 1].type == 0) {
                    te[x+W][y +H- 1].mA += 1;
    ...
    for (int fk = 0; fk < b_m[b_mid].fsize; fk++) {
        if (b_f[fk].check == 1) {
            if (b_f[fk].id == 0) {
                ...
                enemy_type_get(e, e_t, te, b_m, b_mid, 6, 21, 24, 4);
                BeginBatchDraw();
                maps(p, e, b_m, ar, te, b_mid);/*地圖繪製*/
                e_put(e, b_m, te, b_m[b_mid].esize, b_mid);
                b_nput(b_n, b_m, b_m[b_mid].nsize, b_mid);
    ...
    if (chose == "e") {
        ...
        for (t = 0; t < 100; t++) {
            ...
            if (e[bmax].turn > p[pmax].turn && e[bmax].turn >= 100) {
                ...
                if (P_id == 0) {
                     VOplay("shonen6-zyunbihadekiteruyo", 1);
                }
                break;
                }                
            }
                ...
                b_nid = nmax;
                b_n[nmax].turn -= 100;
				b_n[b_nid].move = b_n[b_nid].Move; b_n[b_nid].act = b_n[b_nid].Act;
...
void p_skill(wofstream* wofs, string &chose,player *p,enemy *e,enemy_type *e_t,b_npc *b_n,skill *sk,arms *ar,armor *Ar, t_equip* t_E,item *it,b_map *b_m, terrain(*te)[50], int sk_id,int P_id,int esize,int b_mid,int psize,int bu_id,int b_nid) {
    wstring vos;
    if (chose == "k") {
        int a[100];
    BeginBatchDraw();
    ...
    putimage(0, 0, &m1);
        ...
        if (uk == 1) {
                                    ...
                                    if (em.x > (p[P_id].x + i) * 48-b_m[b_mid].ox && em.x < (p[P_id].x + i) * 48 + 48 - b_m[b_mid].ox && em.y >(p[P_id].y + j) * 48 - b_m[b_mid].oy && em.y< (p[P_id].y + j) * 48 + 48 - b_m[b_mid].oy&&em.x<960&&em.y<720) {
                                        for (int I = 0; I < b_m[b_mid].esize; I++) {
                                            if (e[I].x == p[P_id].x + i && e[I].y == p[P_id].y + j&&em.lbutton) {
                                                wstring Dmg=L"1d4+"+to_wstring((p[P_id].str+p[P_id].dex)/2);
                                                int dmg;
                                                int hcv=PKTVS(p, e, b_m, ar, e_t, P_id, I, b_mid, -1, 2,Dmg);
                                                *wofs << L"(T" << b_m[b_mid].time << L")" << p[P_id].name << L"使用了" << sk[uk].name ;
                                                if (retryp(wofs,p,e,ar,Ar,b_m,P_id,I,-1,b_mid,hcv,2)>0) {
                                                    dmg = roll("1d4", 1, b_m[b_mid].rng) + (p[P_id].str + p[P_id].dex) / 5;
            ...
            while (1) {
                ...
                if (im.lbutton) {
                    for (k = 0; k <= n; k++) {
                        if (im.x >= 175 && im.x <= 325 && im.y >= 799 + 48 * k && im.y <= 847 + 48 * k) {
                            if (k == 0) {
        ...
        while (1) {
            ...
            if (p[P_id].move > 0&&(ar[p[P_id].arms_id_1].type[0]=='m'||ar[p[P_id].arms_id_2].type[0]=='m')) {
                int ar_id;
                if (ar[p[P_id].arms_id_1].type[0] == 'm') {
                    ar_id = p[P_id].arms_id_1;
                }
                else {
                    ar_id = p[P_id].arms_id_2;
                }
                IMAGE p1, mb;
                queue<pair<int, int>> q;               
                wstring mm;
                ...
                while (1) {
                    ...
                    m = getmessage(EM_MOUSE);
                    if (m.lbutton) {
                        *wofs <<L"(T"<<b_m[b_mid].time<<L")" << p[P_id].name << L"開始衝鋒" << endl;
                        for (j = 0; j < 20; j++) {
                            for (i = 0; i < 15; i++) {
                                if (m.x <= j * 48 + 48 && m.x >= j * 48 && m.y <= i * 48 + 48 && m.y >= i * 48 && dr[j + b_m[b_mid].ox / 48][i + b_m[b_mid].oy / 48] == 1 && m.x < 960 && m.y < 720) {
                                    int k, w = 0;
                                    int xbox = p[P_id].x, ybox = p[P_id].y,lm=1;
                                    for (w = 0; w < box_id; w++) {
                                        if (box[w] == 2) {
                                            if (p[P_id].move >= te[xbox][ybox + 1].mA) {
                                                p[P_id].move = p[P_id].move - te[xbox][ybox + 1].mA;
                                        ...
                                        for (int I = -2; I <= 2; I++) {
                                            for (int J = -2; J <= 2; J++) {
                                                ...
                                                if (abs(I )+abs( J) == 1) {
                                                    for (int K = 0; K < b_m[b_mid].esize;K++) {       
                                                        if (xbox +I== e[K].x &&ybox + J == e[K].y) {
                                                        int hcv=PKTVS(p, e, b_m, ar, e_t, P_id, K, b_mid, ar_id, 0,ar[ar_id].Dmg+L"+"+to_wstring(lm/2));
                                                        *wofs << L"對"<<e[K].name<<L"攻擊";
                                                        if (retryp(wofs,p,e,ar,Ar,b_m,P_id,K,ar_id,b_mid,hcv,0)>0) {
                                                            int dmg=roll(ar[ar_id].dmg,1, b_m[b_mid].rng)+lm/2;
                                                            eHP(e, b_m, K, b_mid, 0, dmg);
                                                            *wofs << L"造成" << to_wstring(dmg) << L"點傷害" << endl;
                                                        }
                                                        else {
                                                            gettextstyle(&lft);
                                                            settextstyle(25, 0, _T("Taipei Sans TC Beta"));
                                                            settextcolor(RGB(255, 0, 0));
                                                            mm = L"miss";
                                                            outtextxy(e[K].x * 48 - b_m[b_mid].ox, e[K].y * 48 - b_m[b_mid].oy - 40, mm.c_str());
                                                            settextstyle(&lft);
                                                            settextcolor(RGB(0, 0, 0));
                                                            *wofs << endl;
                                                        }
...
void n_attack(wofstream* wofs, b_npc* b_n, b_map* b_m, enemy* e, player* p, arms* ar, terrain(*te)[50], int b_mid, int b_nid, int& id, int psize, int esize, int bu_id, int P_id) {
    ...
    if (ar[b_n[b_nid].baid].range >= abs(b_n[b_nid].x - e[id].x) + abs(b_n[b_nid].y - e[id].y)) {
        ...
        if (ar[b_n[b_nid].baid].type[0] == 'r') {
            ATK = roll("", 2, b_m[b_mid].rng) + roll(ar[b_n[b_nid].baid].hit, 1, b_m[b_mid].rng) + (b_n[b_nid].dex - 10) / 2;
        }
        else if (ar[b_n[b_nid].baid].type[0] == 'm') {
            ATK = roll("", 2, b_m[b_mid].rng) + roll(ar[b_n[b_nid].baid].hit, 1, b_m[b_mid].rng) + (b_n[b_nid].str - 10) / 2;
        }
        AC = roll("", 2, b_m[b_mid].rng) + (e[id].dex - 10) / 2;
        if (e[id].buff_check[20] == 1) {
            AC = 0;
        ...
        if (ATK > AC) {
            ...
            p_put(p, b_m, psize, b_mid);
            ui(p, e, b_m, P_id, esize, psize, b_mid, bu_id);
            int  DMG = roll(ar[b_n[b_nid].baid].dmg, 1, b_m[b_mid].rng);
...
void e_battle_ai(wofstream* wofs, enemy *e,player *p,e_npc *e_n,b_npc *b_n,b_map*b_m, enemy_type *e_t,arms *ar,armor *Ar,terrain(*te)[50],b_flag *b_f,int id,int P_id,int b_nid,int b_mid,int buff_id) {
    ...
    if (e[id].type == 0) {
                            ...
                            if (I == 2||(I==1&&J==0)) {
                                ...
                                for (int K = 0; K < b_m[b_mid].psize; K++) {
                                    if ((p[K].x == e[id].x + 2 && (p[K].y==e[id].y+1|| p[K].y == e[id].y|| p[K].y == e[id].y - 1))||(p[K].x==e[id].x+1&&p[K].y==e[id].y)) {
                                        int DMG = roll("2d4", 1, b_m[b_mid].rng);
                                        pHP(p, b_m, K, b_mid, 0, DMG);
                                    }
                                }
                                for (int K = 0; K < b_m[b_mid].nsize; K++) {
            ...
            if (e[id].target == -1) {
                ...
                if (e[id].target==0) {
                    ...
                    *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name<<L"對"<<p[P_id].name << L"發起刺刀突擊";
                    ...
                    if (retrye(wofs, p, e, ar, Ar, b_m,P_id, id,b_mid, ro, 1) > 0) {
                    int dmg = roll("1d4", 1, b_m[b_mid].rng);
                    pHP(p, b_m, P_id, b_mid, 0, dmg);
                    }
                    else {
                        settextstyle(25, 0, _T("Taipei Sans TC Beta"));
                    ...
                    if (abs(e[id].x - b_n[b_nid].x) + abs(e[id].y - b_n[b_nid].y) == 1) {
                        skill_show(L"刺刀突擊");               
                        *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name << L"對" << b_n[b_nid].name << L"發起刺刀突擊";
                        if (b_n[b_nid].buff_check[2] > 0) {
                           /*NPC優劣勢處理 暫定*/
                        }
                        if (retryn(wofs, b_n, e, ar, Ar, b_m,b_nid, id,b_mid, 0, 1) > 0) {
                            int dmg = roll("1d4", 1, b_m[b_mid].rng);
                            nHP(b_n, b_m, b_nid, b_mid, 0, dmg);
                        }
                        else {
        ...
        if (e[id].cd[0] == 0) {
            ...
            if (abs(e[id].target_x - e[id].x) + abs(e[id].target_y - e[id].y) < 5&&e[id].act>0) {
        ...
        e_attack(wofs, ar, Ar, p, e, b_n, b_m,te, e_t, b_mid, id, P_id, b_m[b_mid].esize, b_m[b_mid].psize, buff_id, b_nid);
            ...
            if (e[id].cd[1] == 0 && e[id].hp < 25) {
                ...
                e_walk(wofs, e, p, b_n, b_m, ar, te, id, P_id, b_m[b_mid].esize, b_m[b_mid].psize, b_mid, buff_id, b_nid, e[id].target, 0);
                if (abs(p[P_id].x - e[id].x) + abs(p[P_id].y - e[id].y) <= 5&&e[id].act>0) {
    ...
    if (e[id].cd[0] == 100000) {
        if (e[id].pose == 1) {
            ...
            if (e[id].x + e[id].w < b_m[b_mid].x && te[e[id].x + e[id].w][e[id].y].type == 0 && te[e[id].x + e[id].w][e[id].y].npc == 0 && te[e[id].x + e[id].w][e[id].y].enemy == 0 && te[e[id].x + e[id].w][e[id].y].player == 0) {
                enemy_type_get(e, e_t, te, b_m, b_mid, 1, e[id].x + e[id].w, e[id].y, 1);
                e[id].cd[0] = 30;
                skill_show(L"部屬步兵");
            }
        }
        else if (e[id].pose == 3) {
            if (e[id].x -1 >= 0 && te[e[id].x -1][e[id].y].type == 0 && te[e[id].x -1][e[id].y].npc == 0 && te[e[id].x -1][e[id].y].enemy == 0 && te[e[id].x -1][e[id].y].player == 0) {
                enemy_type_get(e, e_t, te, b_m, b_mid, 1, e[id].x-1, e[id].y, 1);
                e[id].cd[0] = 30;
                skill_show(L"部屬步兵");
            }
        }
        else if (e[id].pose == 4) {
            if (e[id].y + e[id].h < b_m[b_mid].y && te[e[id].x][e[id].y + e[id].h].type == 0 && te[e[id].x][e[id].y + e[id].h].npc == 0 && te[e[id].x][e[id].y + e[id].h].enemy == 0 && te[e[id].x][e[id].y + e[id].h].player == 0) {
                enemy_type_get(e, e_t, te, b_m, b_mid, 1, e[id].x, e[id].y + e[id].h, 1);
                e[id].cd[0] = 30;
                skill_show(L"部屬步兵");
            }
        }
    }
    if (e[id].cd[1] == 0&&e[id].target!=-1&&abs(e[id].x-e[id].target_x)+abs(e[id].y-e[id].target_y)>5) {
        e[id].buff_check[35] = 1;
        e[id].buff_time[e[id].buff_Size] = 9;
        e[id].buff_id[e[id].buff_Size] = 35;
        e[id].buff_Size++;
        e[id].cd[1] = 75;
    ...
    else if (e[id].cd[2] == 0&&e[id].target==-1&&e[id].lastx!=-1&&e[id].lasty!=-1&&e[id].act>0 && abs(e[id].x-e[id].lastx) + abs(e[id].y-e[id].lasty) > 5) {
        skill_show(L"發射照明彈");
        te[e[id].lastx][e[id].lasty].flare = 1;
		te[e[id].lastx][e[id].lasty].flare_time = 10;
        for(int I=-5;I<=5;I++){
            for(int J=-5;J<=5;J++){
                if(abs(I)+abs(J)<=5){
                    for (int K = 0; K < b_m[b_mid].psize; K++) {
                        if(p[K].x==e[id].lastx+I&&p[K].y==e[id].lasty+J){
                            p[K].buff_check[38] = 1;
                            p[K].buff_id[p[K].buff_Size] = 38;
                            p[K].buff_time[p[K].buff_Size] = 10;
                            p[K].buff_Size++;
						}
                    }
                    for (int K = 0; K < b_m[b_mid].esize; K++) {
                        if (e[K].x == e[id].lastx + I && e[K].y == e[id].lasty + J) {
                            e[K].buff_check[38] = 1;
                            e[K].buff_id[e[K].buff_Size] = 38;
                            e[K].buff_time[e[K].buff_Size] = 10;
                            e[K].buff_Size++;
                        }
                    }
                    for (int K = 0; K < b_m[b_mid].nsize; K++) {
                        if (b_n[K].x == e[id].lastx + I && b_n[K].y == e[id].lasty + J) {
                            b_n[K].buff_check[38] = 1;
                            b_n[K].buff_id[b_n[K].buff_Size] = 38;
                            b_n[K].buff_time[b_n[K].buff_Size] = 10;
                            b_n[K].buff_Size++;
                        }
                    }
                }
        ...
        if (e[id].target != -1&&e[id].act>0) {
            if (e[id].pose == 1) {
                                            ...
                                            if (e[id].x + J+K >= 0 && e[id].x + J+K < b_m[b_mid].x ) {
                                                for (int L = 0; L < b_m[b_mid].psize; L++) {
                                                    if (e[id].x + J+K == p[L].x && e[id].y + e[id].h + I == p[L].y) {
                                                        *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name << L"用" << ar[e[id].baid].name << L"對" << p[L].name << L"造成";
                                                        int  DMG = roll(ar[e[id].baid].dmg, 1, b_m[b_mid].rng);
                                                        int dev = roll("", 2, b_m[b_mid].rng);
                                                        if (p[L].buff_check[14] == 0) {
                                                            if (p[L].dex + p[L].EDV >= dev) {
                                                                DMG /= 2;
                                                                  *wofs  << DMG << L"點傷害(迴避成功" << dev << "<=" << p[L].dex+p[L].EDV << ")" << endl;
                                                            }
                                                            else {
                                                                *wofs << DMG << L"點傷害(迴避失敗" << dev << ">" << p[L].dex + p[L].EDV << ")" << endl;
                                                            }
                                                        }
                                                        pHP(p,b_m,L,b_mid,0,DMG);
                                                        pD(wofs, p, e, b_m, te, L, b_mid);
                                                    }
                                                }
                                                for (int M = 0; M < b_m[b_mid].nsize; M++) {
                                                    if (e[id].x + J + K == b_n[M].x && e[id].y + e[id].h + I == b_n[M].y) {
                                                        *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name << L"用" << ar[e[id].baid].name << L"對" << b_n[M].name << L"造成";
                                                        int  DMG = roll(ar[e[id].baid].dmg, 1, b_m[b_mid].rng);
                                                        int dev = roll("", 2, b_m[b_mid].rng);
                                                        if (b_n[M].buff_check[14] == 0) {
                                                            if (b_n[M].dex  >= dev) {
                                                                DMG /= 2;
                                                                *wofs << DMG << L"點傷害(迴避成功" << dev << "<=" << b_n[M].dex  << ")" << endl;
                                                            }
                                                            else {
                                                                *wofs << DMG << L"點傷害(迴避失敗" << dev << ">" << b_n[M].dex << ")" << endl;
                                                            }
                                                        }
                        ...
                        if (e[id].x - I - 1 >= 0 && e[id].y + J >= 0 && e[id].y + J < b_m[b_mid].y&&e[id].x-I-1==p[e[id].target_id].x&&e[id].y+J==p[e[id].target_id].y) {
                            ...
                            for (int K = -3; K <= 3; K++) {
                                if ( e[id].y + J + K >= 0 && e[id].y + J + K < b_m[b_mid].y) {
                                    ...
                                    for (int M = 0; M < b_m[b_mid].nsize; M++) {
                                        if (e[id].x -I-1 == b_n[M].x && e[id].y +J+K == b_n[M].y) {
                                            *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name << L"用" << ar[e[id].baid].name << L"對" << b_n[M].name << L"造成";
                                            int  DMG = roll(ar[e[id].baid].dmg, 1, b_m[b_mid].rng);
                                            int dev = roll("", 2, b_m[b_mid].rng);
                                            if (b_n[M].buff_check[14] == 0) {
                ...
                if (e[id].target == 0) {
                    for (int I = 0; I < ar[e[id].baid].range; I++) {
                        for (int J = -I; J < I + e[id].h; J++) {
                            if (e[id].x +e[id].w + I >= 0 && e[id].y + J >= 0 && e[id].y + J < b_m[b_mid].y && e[id].x + e[id].w + I == p[e[id].target_id].x && e[id].y + J == p[e[id].target_id].y) {
                                        b_camera(b_m, p[e[id].target_id].x, p[e[id].target_id].y,b_mid);      
                                        BeginBatchDraw();
                                        Redraw_battle(p, e, b_n, ar, te, b_m, b_mid);
                                        for (int K = -3; K <= 3; K++) {
                                            if (e[id].y + J + K >= 0 && e[id].y + J + K < b_m[b_mid].y) {
                                                ...
                                                for (int M = 0; M < b_m[b_mid].nsize; M++) {
                                                    if (e[id].x + e[id].w + I == b_n[M].x && e[id].y + J + K == b_n[M].y) {
                                                        *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name << L"用" << ar[e[id].baid].name << L"對" << b_n[M].name << L"造成";
                                                        int  DMG = roll(ar[e[id].baid].dmg, 1, b_m[b_mid].rng);
                                                        int dev = roll("", 2, b_m[b_mid].rng);
                                                            if (b_n[M].dex >= dev) {
                            ...
                            if (e[id].x + J >= 0 && e[id].x + J < b_m[b_mid].x && e[id].y - I - 1 >=0&& e[id].x + J == p[e[id].target_id].x && e[id].y - I - 1 == p[e[id].target_id].y) {
                                    ...
                                    for (int K = -3; K <= 3; K++) {
                                        if (e[id].x + J + K >= 0 && e[id].x + J + K < b_m[b_mid].x ) {
                                            ...
                                            for (int M = 0; M < b_m[b_mid].nsize; M++) {
                                                if (e[id].x + J + K == b_n[M].x && e[id].y - I - 1 == b_n[M].y) {
                                                    *wofs << L"(T" << b_m[b_mid].time << L")" << e[id].name << L"用" << ar[e[id].baid].name << L"對" << b_n[M].name << L"造成";
                                                    int  DMG = roll(ar[e[id].baid].dmg, 1, b_m[b_mid].rng);
                                                    int dev = roll("", 2, b_m[b_mid].rng);
                                                    if (b_n[M].buff_check[14] == 0) {
...
int main() {
    HWND hwnd = initgraph(1296+16, 999, EX_SHOWCONSOLE);
    g_OriginWndProc = (WNDPROC)GetWindowLongPtr(hwnd, GWLP_WNDPROC);
    SetWindowLongPtr(hwnd, GWLP_WNDPROC, (LONG_PTR)WindowProc);
    SetWindowPos(hwnd, HWND_TOP, 312, 30, 1296+16, 999, 0);
    settextcolor(WHITE);
    setbkmode(TRANSPARENT);
    settextstyle(30, 0, _T("Taipei Sans TC Beta"));
    ...
    spec.channels = 2;
    g_mixer = MIX_CreateMixerDevice(
        SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK,
        &spec
    );
    for ( i = 0; i < SE_TRACK_COUNT; i++) {
        g_seTrack[i] = MIX_CreateTrack(g_mixer);
    }
    for (i = 0; i < VO_TRACK_COUNT; i++) {
        g_voTrack[i] = MIX_CreateTrack(g_mixer);
    }
    int  variable = 1, id = 0, P_id = 0, m_id = 1, psize = 1, load = 0, n_id = 1, i_id = 18, ar_id = 12, Ar_id = 3, st_id = 3, f_id = 22, b_mid, ex_id = 14, b_id = 9, sk_id = 22, buff_id = 40, m_fid = 11, b_nid = 0, t_Eid = 3, sp_id = -1,tk_id=6;
    /*變數數量*/    
    wofstream wofs;
    player p[3];
    enemy_type e_t[8];
...
#pragma region 
    ...
    m[9].nsize = 0;  m[9].esize = 0; m[9].exitsize = 1; m[9].exit_set = "13_"; m[9].mevent_size = 3; m[9].mevent_set = "7_8_9_"; m[9].name = L"星落村村長家";
    b_m[0].esize = 1; b_m[0].e_set = "e0x6y4p1"; b_m[0].p_set = "x18y14x19y14"; b_m[0].cx = 0; b_m[0].cy = 0; b_m[0].fsize = 0; b_m[0].nsize = 0;
    b_m[1].esize = 1; b_m[1].e_set = "e7x21y23p1"; b_m[1].p_set = "x21y26x21y27x21y28"; b_m[1].cx = 27; b_m[1].cy = 26; b_m[1].fsize = 4; b_m[1].f_set = "0n1n6n7n"; b_m[1].type = 1;
    b_m[2].esize = 2; b_m[2].e_set = "e0x5y4p1e0x6y3p1"; b_m[2].p_set = "x18y13x19y13"; b_m[2].cx = 0; b_m[2].cy = 0; b_m[2].fsize = 1; b_m[2].f_set = "2n"; b_m[2].nsize = 1; b_m[2].n_set = "n0x16y10";b_m[2].lc = L"1.我方全員被擊敗\n2.戴恩或愛德華被擊敗"; b_m[2].vc = L"1.完成愛麗絲的指示";
    ...
    e_t[0].name = L"野狼"; e_t[0].story = L"群體行動的動物 隨著數量增加危險性也會大幅上升"; e_t[0].baid = 1; e_t[0].str = 12; e_t[0].dex = 13; e_t[0].con = 12; e_t[0].INT = 3; e_t[0].wis = 14; e_t[0].cha = 7; e_t[0].lv = 1; e_t[0].mhp = 11; e_t[0].hp = 11; e_t[0].Move = 7; e_t[0].speed = 12; e_t[0].exp = 105; e_t[0].drop = "100%15i"; e_t[0].species = "a";
    ...
    e_t[2].name = L"野蜂"; e_t[2].story = L"領地意識極強，一但靠近就會使用帶有毒液的尾針攻擊"; e_t[2].baid = 7; e_t[2].str = 3; e_t[2].dex = 15; e_t[2].con = 10; e_t[2].INT = 1; e_t[2].wis = 7; e_t[2].cha = 1; e_t[2].lv = 1; e_t[2].mhp = 5; e_t[2].hp = 5; e_t[2].Move = 5; e_t[2].speed = 15; e_t[2].exp = 50; e_t[2].drop = "20%16i"; e_t[2].species = "a"; e_t[2].e_range = 8;
    ...
    ar[9].name = L"短嚎"; ar[9].dmg = "3d4"; ar[9].Dmg = L"3d3"; ar[9].hit = "3d3"; ar[9].story = L"帝國士官配發的防衛手槍，能全自動射擊"; ar[9].type = "rp";
    ...
    ar[11].name = L"帝國車載輕機槍"; ar[11].dmg="2d6"; ar[11].Dmg=L"2d6"; ar[11].range = 5; ar[11].story = L"帝國為載具對抗輕目標所設計的小口徑機槍，通用可卸的設計使其適用於絕大部分戰場上\n傷害:2d6 裝填時間:8S"; ar[11].type = "rr"; ar[11].mbullet = 100;
    ...
    it[2].Name = L"通常彈"; it[2].name = "通常彈"; it[2].number = 0; it[2].type = 'b'; it[2].story = L"";
    ...
    it[7].Name = L"巴平"; it[7].name = "巴平"; it[7].number = 0; it[7].type = 'm'; it[7].story = L"巴蘭斯建國後鑄造的銀幣，巴蘭斯國的貨幣最大單位"; it[7].value = 1000;
    ...
    it[17].Name = L"燃燒瓶"; it[17].name = "燃燒瓶"; it[17].number = 0; it[17].type = 'a'; it[17].story = L"裝有易燃液體的玻璃瓶，製作簡單且對抗重甲單位有奇效\n投擲物 對目標範圍內的單位造成2d3點傷害以及3層燃燒，同時點燃地面"; it[17].value = 200;
    ...
    sk[0].name = L"戰場敏銳"; sk[0].story = L"較高的戰術素養能抓準敵人攻勢的破綻\n被動:每30秒為我方全員取得一層戰術優勢"; sk[0].type = "b0"; sk[0].cost = 3;
    ...
    sk[5].name = L"傷患轉移"; sk[5].story = L"將傷患轉移到更適合治療的地點\n效果:消耗1AP，支付所需MP轉移單位與自身到指定位置"; sk[5].type = "A"; sk[5].cost = 2;
    sk[6].name = L"白之祝福"; sk[6].story = L"為一名友方單位注入生命之力\n效果:消耗1AP，給予5+(WIS/2)的護盾，以及60秒的(WIS/7)層純白庇護"; sk[6].type = "B"; sk[6].cost = 3;
    ...
    sk[13].name = L"步槍掌握"; sk[13].story = L"熟悉了步槍的運作，能命中更遠的目標\n被動:使用步槍時，射程和命中+1"; sk[13].cost = 3; sk[13].type = "b23";
    sk[14].name = L"隱匿者"; sk[14].story = L"通過隱匿技巧避免進入敵方視線，實現難以防備的攻擊\n被動:進入戰鬥後第一次攻擊必定命中，必須使用遠程武器，當(30-DEX/2)秒內未主動攻擊，將可以再觸發本被動"; sk[14].cost =2 ; sk[14].type = "b25";
    ...
    sk[17].name = L"野性洞察"; sk[17].story = L"狩獵的經驗使得對野獸的習性相當熟悉，能更好的找到獵物的破綻\n被動:對非人類動物單位攻擊判定自帶一層優勢，最終傷害提升30%"; sk[17].cost =3 ; sk[17].type = "P";
    sk[18].name = L"狩獵陷阱"; sk[18].story = L"對獵人而言，陷阱的設置是一門重要的技術\n效果:花費10秒的時間設置一個捕獸夾，觸發陷阱的敵人中斷當前移動並會受到5點傷害以及獲得60秒足傷"; sk[18].cost = 2; sk[18].type = "A";
    ...
    bu[1].name = L"攻擊優勢"; bu[1].story = L"普攻前可消耗N層，若普攻失敗可重新判定N次";
    bu[2].name = L"戰術優勢"; bu[2].story = L"普攻或閃避前可消耗N層，若失敗可重新判定N次";
    ...
    bu[7].name = L"純白庇護"; bu[7].story = L"每一層可以減少一點所受的生命傷害";
    bu[8].name = L"星隕護盾"; bu[8].story = L"當單位受到傷害時扣除等量護盾減少傷害，但若攻擊者存在星隕護盾並以近戰攻擊，則無法減免傷害";
    ...
    bu[12].name = L"燒灼"; bu[12].story = L"回合開始時造成MHP5%*層數的直接傷害(最少為1)，並減少一層燒灼";    
    bu[13].name = L"蓄力中-火球"; bu[13].story = L"對3×3範圍內的單位造成2d4的範圍傷害以及附加一層燒灼，並燃燒地面60秒，射程4";
    bu[14].name = L"力竭"; bu[14].story = L"無法閃避以及行動，若再次受到攻擊或倒數結束，將會退出戰鬥，當HP回到1以上將解除力竭狀態";
    bu[15].name = L"蓄力中-炎浪"; bu[15].story = L"對範圍內的單位造成3d3+燒灼層數×2的傷害，並燃燒地面90秒";
    ...
    bu[22].name = L"哨戒"; bu[22].story = L"當敵人在武器射程內進行移動或行動前，先進行一次攻擊，命中後使移動力歸0，優先使用射程較遠的武器，彈藥不足將無法觸發";
    bu[23].name = L"步槍掌握"; bu[23].story = L"步槍射程+1";
    bu[24].name = L"隱匿者:冷卻中"; bu[24].story = L"持續時間內如不進行普攻，冷卻完將能獲得一層隱匿者";
    bu[25].name = L"隱匿者"; bu[25].story = L"普攻必定命中，必須使用槍械";
    ...
    bu[29].name = L"戰術劣勢"; bu[29].story = L"成功進攻或閃避時，自動移除一層劣勢並重新判定，直到失敗或劣勢耗盡";
    bu[30].name = L"進攻劣勢"; bu[30].story = L"進攻成功時，自動移除一層劣勢並重新判定，直到失敗或劣勢耗盡";
    bu[31].name = L"中毒-蜂毒"; bu[31].story = L"回合開始時進行體質判定，成功則移除一層，否則將受到3點直接傷害";
    bu[32].name = L"駐守"; bu[32].story = L"單位尚未發現敵人，將在原地駐守直到發現敵人";
    bu[33].name = L"索敵中"; bu[33].story = L"單位未找到敵人，將前往最後發現敵人蹤跡的位置";
    bu[34].name = L"破障Lv1";bu[34].story = L"移動時能清除小型障礙與陷阱";
    bu[35].name = L"迫擊炮發射中";bu[35].story = L"進行每3秒一發的火力覆蓋，落地點造成3×3範圍4d3點傷害，並施加一層震懾";
    bu[36].name = L"震懾";bu[36].story = L"回合開始時進行體質判定，若失敗則AP-1，並移除一層該被動";
    bu[37].name = L"金屬造物";bu[37].story = L"由金屬構成，常規的對人手段無法造成影響或傷害";
    bu[38].name = L"目眩"; bu[38].story = L"因強光影響而視線模糊，攻擊與迴避判定皆受一個懲罰骰";
    bu[39].name = L"車燈"; bu[39].story = L"照亮前方扇型範圍，自身無法隱匿";
...
/*開始畫面*/
    ...
    FP = 0;
        ...
        m_set(m, n, p, e_n,f, Box, m_id, b_id);m_map(p,m, Box, m_id, b_id, n,""); m_put(p, m, m_id);;
            string g;
            Z = 2;
...
/*一般模式*/
        while (1) {
            if (isReady) {
                ...
                if (Z == 2) {
                    m_check(p, m, EX,f, Box, m_id,b_id, n, e_n);
                    FlushConsoleInputBuffer(GetStdHandle(STD_INPUT_HANDLE));
                    flushmessage(-1);
                }
                /*!=-1*/
...
/*戰鬥模式*/
        ...
        while (1) {
            ...
            if (roundp == 1) {
                ...
                p_skill(&wofs,chose,p,e,e_t,b_n,sk,ar,Ar,t_E,it,b_m,te,sk_id,P_id,b_m[b_mid].esize,b_mid,b_m[b_mid].psize,buff_id,b_nid);
